...
const Navbar = () => {
    ...
    const isActivePage = (url: string, hash: string = '') => {
        if (hash && pathname === '/') return currentHash === hash;
        ...
        <nav
            ...
            <div className='mx-auto flex h-14 max-w-7xl items-center px-4'>
                ...
                <div className='flex flex-1 items-center justify-start'>
                    <Link
                        href='/'
                        className='flex items-center transition-all duration-200 hover:scale-105 hover:text-emerald-700'>
                            ...
                            {item.title === 'Analytics Tools' ? (
                                <div ref={dropdownRef} className='relative'>
                                    <button
                                        ...
                                        className={`flex transform items-center gap-1 text-[0.95rem] font-medium tracking-wider transition-all duration-200 hover:scale-105 hover:text-emerald-700 ${
                                            ...
                                            <div className='p-2'>
                                                {analyticsTools.map((tool) => (
                                                    <Link
                                                        ...
                                                        className='flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-gray-700 transition-all duration-200 hover:scale-[1.02] hover:bg-emerald-50 hover:text-emerald-700'>
                                ...
                                <Link
                                    href={item.url}
                                    className={`transform text-[0.95rem] font-medium tracking-wider transition-all duration-200 hover:scale-105 hover:text-emerald-700 ${
                                        isActivePage(item.url, item.hash)
                                            ? 'font-semibold text-emerald-600 drop-shadow-[0_0_8px_rgba(16,185,129,0.4)]'
                                            : 'text-gray-700'
                                    }`}>
                ...
                <div className='flex flex-1 items-center justify-end gap-3'>
                    <Button
                        ...
                        variant='outline'
                        size='sm'
                        className='hidden transform rounded-full border-emerald-600 text-emerald-700 transition-all duration-200 hover:scale-105 hover:bg-emerald-50 hover:text-emerald-800 md:inline-flex'>
                    ...
                    <Button
                        asChild
                        size='sm'
                        className='hidden transform rounded-full bg-emerald-600 text-white transition-all duration-200 hover:scale-105 hover:bg-emerald-700 md:inline-flex'>
